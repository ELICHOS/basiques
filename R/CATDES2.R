#' @export catdes2
catdes2<-function (donnee=dataRPV.numok[, 1:17], num.var=1, proba = 0.05, row.w = NULL) 
{
  #donnee=dataRPV.numok[, c(1:16, 18, 19)]
  ...
  if (!is.null(quanti)) {
    ...
    ####
    testFunction <- function (xi) {
      return(tryCatch(summary(aov(donnee[, quanti[i]] ~ donnee[, 
                                                               num.var], na.action = na.exclude, weights = row.w))[[1]], error=function(e) NULL))
    }
    ####
    for (i in 1:length(quanti)) {
      #tryCatch({
      res.aov <- testFunction(i)#, 
      #error=function(e){cat("ERROR :",conditionMessage(e), "\n")}
      #})
      message(paste("JITVB: ", i))
      if(!is.null(res.aov)){
