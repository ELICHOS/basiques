HCPC_dissi<-function (res, nb.clust = 0, consol = TRUE, iter.max = 10, min = 3, 
          ...
          cluster.CA = "rows", kk = Inf, description = TRUE, ...) 
{
  auto.cut.tree = function(res, min, max, metric, method, 
                           weight = NULL, cla = NULL, ...) {
    if (order) {
      if (is.null(res$call$row.w)) 
        res$call$row.w = rep(1/nrow(res$ind$coord), 
                             nrow(res$ind$coord))
    ...
    return(list(res = res, tree = hc, nb.clust = nb.clust, 
                within = intra, inert.gain = inert.gain, quot = quot, dissi=dissi, do=do))
  }
  ...
  distinctivness = function(Y, default.size, method, coord.centers) {
    ...
    if (nrow(Y) == 1) {
      ...
      else center.min <- apply(distance[-clust, ], 2, 
                               min)
  ...
  if (inherits(res, "CA")) {
    ...
    if (cluster.CA == "rows") 
      res <- PCA(res$row$coord, scale.unit = FALSE, ncp = Inf, 
                 graph = FALSE, row.w = res$call$marge.row * 
                   sum(res$call$X))
    if (cluster.CA == "columns") 
      res <- PCA(res$col$coord, scale.unit = FALSE, ncp = Inf, 
                 graph = FALSE, row.w = res$call$marge.col * 
                   sum(res$call$X))
    res$eig <- aux
  }
  if (is.null(max)) 
  ...
  if (inherits(t$tree, "hclust")) {
    ...
    if (graph) {
      ...
      barplot(t$inert.gain[1:max(15, max)], col = c(rep("black", 
                                                        t$nb.clust - 1), rep("grey", max(max, 15) - 
                                                                               t$nb.clust + 1)), rep(0.1, max(max, 15)), space = 0.9)
      plot(x = 1, xlab = "", ylab = "", main = "", col = "white", 
           axes = FALSE)
      text(1, 1, "Hierarchical Clustering", cex = 2)
      plot(x = 1, xlab = "", ylab = "", main = "", col = "white", 
           axes = FALSE)
      legend("top", "inertia gain  ", box.lty = NULL, 
             cex = 1)
    }
    else {
    ...
    if ((nb.clust == 0) | (nb.clust == 1)) {
      ...
      if (nb.clust < 0) 
        y = auto.haut
      else y = (t$tree$height[length(t$tree$height) - 
                                nb.clust + 2] + t$tree$height[length(t$tree$height) - 
                                                                nb.clust + 1])/2
    }
  }
  ...
  if (description) {
    if (!inherits(res.sauv, "CA") & !(vec)) {
      desc.var <- catdes(data.clust, ncol(data.clust), 
                         proba = proba, row.w = res.sauv$call$row.w.init)
    }
    else {
      if ((vec) | (is.null(res.sauv$call$quanti.sup) & 
                   is.null(res.sauv$call$quali.sup))) 
        desc.var <- descfreq(data.clust[, -which(sapply(data.clust, 
                                                        is.factor))], data.clust[, ncol(data.clust)], 
                             proba = proba)
      else {
        desc.var <- catdes(data.clust[, c(res.sauv$call$quanti.sup, 
                                          res.sauv$call$quali.sup, ncol(data.clust))], 
                           length(c(res.sauv$call$quanti.sup, res.sauv$call$quali.sup, 
                                    ncol(data.clust))), proba = proba, row.w = apply(data.clust[, 
                                                                                                -c(res.sauv$call$quanti.sup, res.sauv$call$quali.sup, 
                                                                                                   ncol(data.clust))], 1, sum))
        desc.var$frequency <- descfreq(data.clust[, 
                                                  -c(res.sauv$call$quanti.sup, res.sauv$call$quali.sup, 
                                                     ncol(data.clust))], data.clust[, ncol(data.clust)], 
                                       proba = proba)
        desc.var <- desc.var[c(length(desc.var), 1:(length(desc.var) - 
                                                      1))]
      }
    }
    if (kk == Inf) 
      desc.axe <- catdes(X, ncol(X), proba = proba, row.w = res$call$row.w.init)
  }
  if (inherits(res.sauv, "data.frame")) 
  ...
  if (consol) 
    call <- list(t = t, min = min, max = max, X = X, bw.before.consol = sum(rev(t$tree$height)[1:(nb.clust - 
                 ...
                 vec = vec, call = match.call())
  else call <- list(t = t, min = min, max = max, X = X, bw.before.consol = sum(rev(t$tree$height)[1:(nb.clust - 
                                                                                                       1)]), vec = vec, call = match.call())
  if (description) {
    if (kk != Inf) 
      res.HCPC <- list(data.clust = data.clust, desc.var = desc.var, 
                       desc.ind = desc.ind, call = call, dissi=t$dissi, do=t$do)
    else res.HCPC <- list(data.clust = data.clust, desc.var = desc.var, 
                          desc.axes = desc.axe, desc.ind = desc.ind, call = call, dissi=t$dissi,do=t$do)
  }
  else {
    res.HCPC <- list(data.clust = data.clust, call = call, dissi=t$dissi,do=t$do)
  }
  if ((kk == Inf) & (graph)) {
